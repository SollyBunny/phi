...
// alias
{
	...
	Vec.add = (...vecs) => { const out = [0, 0]; for (const vec of vecs) { out[0] += vec[0]; out[1] += vec[1] } return out };
	Vec.iadd = (out, ...vecs) => { for (const vec of vecs) { out[0] += vec[0]; out[1] += vec[1] } return out };
	Vec.sub = (...vecs) => { const out = [0, 0]; for (const vec of vecs) { out[0] -= vec[0]; out[1] -= vec[1] } return out };
	Vec.isub = (out, ...vecs) => { for (const vec of vecs) { out[0] -= vec[0]; out[1] -= vec[1] } return out };
	Vec.mul = (a, n) => { return [a[0] * n, a[1] * n]; };
	Vec.imul = (a, n) => { a[0] *= n; a[1] *= n; return a; };
	Vec.div = (a, n) => { return [a[0] / n, a[1] / n]; };
	Vec.idiv = (a, n) => { a[0] /= n; a[1] /= n; return a; };
	Vec.sqrt = (a) => { return [Math.sqrt(a[0]), Math.sqrt(a[1])]; };
	Vec.isqrt = (a) => { a[0] = Math.sqrt(a[0]); a[1] = Math.sqrt(a[1]); return a; };
	Vec.cbrt = (a) => { return [Math.cbrt(a[0]), Math.cbrt(a[1])]; };
	Vec.icbrt = (a) => { a[0] = Math.cbrt(a[0]); a[1] = Math.cbrt(a[1]); return a; };
	Vec.pow = (a, n) => { return [a[0] ** n, a[1] ** n]; };
	Vec.ipow = (a, n) => { a[0] **= n; a[1] **= n; return a; };
	Vec.mid = (...vecs) => { return Vec.idiv(Vec.add(...vecs), vecs.length); };
	Vec.distSquared = (a, b) => { const dx = a[0] - b[0]; const dy = a[1] - b[1]; return dx * dx + dy * dy; };
	Vec.dist = (a, b) => { return Math.sqrt(Vec.distSquared(a, b)); };
	Vec.magSquared = (a) => { return a[0] * a[0] + a[1] * a[1]; };
	Vec.mag = (a) => { return Math.sqrt(Vec.magSquared(a)); };
	Vec.set = (a, x, y) => { a[0] = x; a[1] = y; return a; };
	Vec.copy = (a, b) => { a[0] = b[0]; a[1] = b[0]; return a; };
	Vec.clone = (a) => { return [a[0], a[1]]; };

	window.Body = {};
	Body.Circle = (pos, r, opts) => {
		const args = {
			position: pos,
			radius: r
		};
		...
		body.addShape(new P2.Circle(args));
		return body;
	};
	Body.Rect = (pos, w, h, opts) => {
		const args = {
			position: pos,
			width: w,
			height: h
		...
		if (opts) Object.assign(args, opts);
		const body = new P2.Body();
		body.addShape(new P2.Box(args));
		return Body;
	};
	Body.Plane = (pos, angle, opts) => {
		const args = {
			position: pos,
			angle: angle
		};
		if (opts) Object.assign(args, opts);
		const body = new P2.Body();
		body.addShape(new P2.Plane(args));
		return body;
	};
	Body.Polygon = (pos, radius, sides, opts) => {
		const verts = [];
		for (let i = 0; i < sides; ++i) {
			verts.push(Vec(
				Math.sin(i / sides * 2 * Math.PI) * radius,
				Math.cos(i / sides * 2 * Math.PI) * radius
			));
		}
	}
}
...
function frame() {
	...
	timing.delta = timing.last - now;
	timing.last = now;
	timing.fps = 1000 / timing.delta;
    world.step(1 / 60, timing.delta, 10);
	render();
}
